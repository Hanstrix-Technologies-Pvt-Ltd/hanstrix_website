...
export default function WhyEssential() {
  return (
    // Reduced bottom padding: pb-6 md:pb-8 (was py-12 md:py-16)
    <section className="px-6 lg:px-20 pt-12 md:pt-16 pb-6 md:pb-8">
      ...
      <div className="hidden sm:block max-w-5xl mx-auto">
        <div className="relative">
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-cyan-500/0 via-cyan-500/40 to-purple-500/0" />
      ...
      {/* Mobile: same cards stacked vertically, equal size */}
        ...
        <ul className="space-y-3">
          {whyEssential.map((text, i) => (
            <motion.li
              ...
              className="h-full"
            >
              {/* Equal height: adjust min-h value if you want tighter/looser */}
              <div className="relative rounded-xl border border-white/10 bg-white/[0.06] backdrop-blur-md shadow-[0_10px_28px_rgba(0,0,0,0.22)] px-4 py-3 min-h-[96px] flex items-center">
                <div
                  aria-hidden
                  className="pointer-events-none absolute inset-x-3 -top-1 h-[3px] rounded-full bg-gradient-neon"
                />
                <div className="flex items-start gap-3">
